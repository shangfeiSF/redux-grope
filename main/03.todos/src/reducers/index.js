 ...
 * Redux提供combineReducers，combineReducers只是生成一个函数，这个函数来调用每一个reducer
 * 调用每个reducer时
...
export default combineReducers({
  addAndToggle, // combineReducers为addAndToggle在全局state中分配的key就是addAndToggle
  filter // combineReducers为filter在全局state中分配的key就是filter
  /*
   * state对象：{
   *   addAndToggle: [],
   *   filter: ''
